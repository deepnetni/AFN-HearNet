...
% Function to design an FIR NAL-R equalization filter（NAL_R FIR滤波器） and a flat filter
...
% Design a flat filter having the same delay as the NAL-R filter（设计与NAL R同时延的平坦滤波器）
delay=zeros(1,nfir+1);
delay(1+nfir/2)=1.0;
    
% Design the NAL-R filter for HI listener（NAL R FIR滤波器）
