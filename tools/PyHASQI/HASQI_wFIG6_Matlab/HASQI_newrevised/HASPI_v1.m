...
%           covariance（协方差） transformed into the estimated intelligibility
...
% ---------------------------------------
% Smooth the envelope outputs（平滑耳蜗模型的返回包络）: 125 Hz sub-sampling rate
segsize=16; %Averaging segment size in msec 平均段大小（16ms的帧）
xdB=eb_EnvSmooth(xenv,segsize,fsamp);
ydB=eb_EnvSmooth(yenv,segsize,fsamp);

% Mel cepstrum correlation using smoothed envelopes 用于平滑包络的Mel倒谱相关
% m1=ave of coefficients 2-6
% xy=vector of coefficients 1-6
thr=2.5; %Silence threshold（静音阈值）: sum across bands, dB above aud threshold（听觉门限）
addnoise=0.0; %Additive noise in dB SL to condition cross-covariances（条件互协方差）
[CepCorr,xy]=eb_melcor(xdB,ydB,thr,addnoise);

% ---------------------------------------
% Temporal fine structure correlation measurements
% Compute the time-frequency segment covariances（计算时频段的协方差）
segcov=16; %Segment size for the covariance calculation（计算协方差的段长）
