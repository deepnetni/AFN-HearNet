...
% Initialize the adaptation time constants 初始化自适应时间常数
tau1=2; %Rapid adaptation in msec 快速自适应ms
tau2=60; %Short-term adaptation in msec 短时自适应ms
tau1=0.001*tau1; tau2=0.001*tau2; %Convert to seconds 换算成s

% Equivalent circuit parameters 等效电路参数
...
% Intermediate values（中间值） used for the voltage update matrix（电压更新矩阵）
% inversion （逆变换）
...
% Initialize the outputs（输出初始化） and state of the equivalent circuit（等效电路）
nsamp=length(xdB);
gain=ones(size(xdB)); %Gain vector to apply to the BM motion（应用于BM震动的增益）, default is 1
ydB=zeros(size(xdB)); %Assign storage
V1=0.0;
V2=0.0;
small=1.0e-30;

% Loop to process the envelope signal 遍历包络进行处理
