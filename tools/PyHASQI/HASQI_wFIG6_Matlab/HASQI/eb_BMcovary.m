...
% Function to compute the cross-covariance（互协方差） (normalized cross-correlation归一化的互相关) 
...
% sigMSx    matrix [nchan,nseg] of MS input signal energy values（能量矩阵）
...
% Lag（滞后） for computing the cross-covariance
lagsize=1.0; %Lag (+/-) in msec
maxlag=round(lagsize*(0.001*fsamp)); %Lag in samples

% Compute the segment window（分段窗）
...
wincorr=1./xcorr(window,window,maxlag); %Window autocorrelation, inverted（窗函数自相关的倒数）
winsum2=1.0/sum(window.^2); %Window power, inverted （窗函数能量的倒数）
...
%	The first (half) windowed segment
	nstart=1;
    segx=x(nstart:nhalf).*halfwindow; %Window the reference 参考信号加窗
    segy=y(nstart:nhalf).*halfwindow; %Window the processed signal 处理信号加窗
    segx=segx - mean(segx); %Make 0-mean 0均值
    segy=segy - mean(segy);
    MSx=sum(segx.^2)*halfsum2; %Normalize signal MS value by the window （使用窗函数对MS信号进行归一化）
    MSy=sum(segy.^2)*halfsum2;
    Mxy=max(abs(xcorr(segx,segy,maxlag).*halfcorr)); %Unbiased cross-correlation 无偏互相关
...
% Adjust the BM magnitude to correspond to（与之对应） the envelope in dB SL
sigMSx=2.0*sigMSx;
sigMSy=2.0*sigMSy;
end
