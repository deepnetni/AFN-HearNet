...
% Function to apportion（分配） the hearing loss to the outer hair cells (OHC)
% and the inner hair cells (IHC) and to increase the bandwidth of the
% cochlear filters in proportion（占比） to the OHC fraction of the total loss.
%
% Calling variables:
...
%
% Returned values:
% attnOHC	attenuation（衰减） in dB for the OHC gammatone filters
% BW		OHC filter bandwidth expressed in terms of normal
% lowknee	Lower kneepoint（下拐点） for the low-level linear amplification（低阶线性放大）
% CR		Ranges from 1.4:1 at 150 Hz to 3.5:1 at 8 kHz for normal
%			hearing. Reduced in proportion to the OHC loss to 1:1.（OHC受损的压缩比例）
% attnIHC	attenuation in dB for the input to the IHC synapse（突触）
%
...
% Interpolation（插值） to give the loss at the gammatone center frequencies
...
fv=[cfreq(1),aud,cfreq(nfilt)]; %Frequency vector for the interpolation（用于插值得频率向量）
...
for n=1:nfilt
	if loss(n) < thrOHC(n)
		...
		attnIHC(n)=0.2*thrOHC(n) + (loss(n)-thrOHC(n)); % 合理？
	end
end
...
% Compute the compression lower kneepoint（下拐点） and compression ratio
