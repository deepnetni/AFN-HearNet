...
% Function to compute the average cross-covariance（平均协方差） between the reference
% and processed signals in each auditory band. The silent time-frequency
% tiles are removed from consideration（去除静音段）. The cross-covariance is computed
...
% covariance values across time and frequency（时频范围的协方差之和） are then divided by the total
% number of tiles above thresold（有声段的总数）. The calculation is a modification of
% Tan et al. (JAES 2004). The cross-covariance is also output with a
% frequency weighting that reflects the loss of IHC synchronization（IHC同步损失） at high
