...
% cf		filter center frequency in Hz（滤波器的中心频率）
%
% Returned values:
% envx      filter envelope output (modulated down to baseband 调制到基带) 1st signal
...
% Initialize the complex demodulation（复解调）
npts=length(x);
cn=cos(tpt*cf);
sn=sin(tpt*cf);
coscf=zeros(npts,1);
